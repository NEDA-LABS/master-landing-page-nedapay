    ...
    <div className="max-w-none">
      <ReactMarkdown
        ...
        components={{
          ...
          // Unwrap paragraphs that hold only an image, so the <figure> below
          // isn't nested inside a <p> (invalid HTML → hydration warnings).
          p: ({ node, children, ...rest }) => {
            const kids = node?.children ?? [];
            if (kids.length === 1 && (kids[0] as { tagName?: string }).tagName === 'img') {
              return <>{children}</>;
            }
            return <p className="text-[15px] leading-relaxed text-slate-700 dark:text-slate-300 font-light mb-5" {...rest}>{children}</p>;
          },
          ...
          img: ({ src, alt }) => (
            <figure className="my-7">
              ...
              <img src={typeof src === 'string' ? src : ''} alt={alt || ''} className="w-full rounded-md" />
              {alt ? (
                <figcaption className="mt-2 text-center text-[12px] font-mono text-slate-400 dark:text-slate-500">
                  {alt}
                </figcaption>
              ) : null}
            </figure>
