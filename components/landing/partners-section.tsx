...
function Testimonials() {
  return (
    <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-20 mb-4">
      {/* Section label */}
      ...
      <div className="mb-10">
        ...
        <p className="text-slate-500 dark:text-slate-500 text-sm mt-2 font-light max-w-lg">
          From sourcing in Guangzhou to sending home from London — borderless payments, working in the real world.
        </p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-5">
        {testimonials.map((t, i) => (
          <div key={i} className="group relative overflow-hidden bg-slate-50 dark:bg-white/[0.025] hover:bg-blue-50/40 dark:hover:bg-blue-500/[0.04] transition-all duration-300 p-6 sm:p-7">
            {/* Top gradient accent */}
            ...
            {/* Corner brackets */}
            <div className={`absolute top-0 left-0 w-4 h-4 border-t border-l transition-colors duration-300 ${t.hue === 'cyan' ? 'border-cyan-500/15 group-hover:border-cyan-400/55' : 'border-blue-500/15 group-hover:border-blue-400/55'}`} />
            <div className={`absolute bottom-0 right-0 w-4 h-4 border-b border-r transition-colors duration-300 ${t.hue === 'cyan' ? 'border-cyan-500/15 group-hover:border-cyan-400/55' : 'border-blue-500/15 group-hover:border-blue-400/55'}`} />

            {/* Top row: tag + flag route */}
            <div className="flex items-center justify-between mb-5">
              <span className={`text-[9px] font-mono tracking-[0.18em] px-2 py-0.5 uppercase ${
                t.hue === 'cyan'
              ...
              <span className="text-xl tracking-widest" title={t.flag}>{t.flag}</span>
            </div>

            {/* Quote mark */}
            <div className={`text-3xl font-serif leading-none mb-3 ${t.hue === 'cyan' ? 'text-cyan-400/40' : 'text-blue-400/40'}`}>"</div>

            {/* Quote */}
            <p className="text-sm sm:text-[15px] text-slate-700 dark:text-slate-300 leading-relaxed font-light mb-6">
              {t.quote}
            </p>

            {/* Bottom: name + metric */}
            <div className="flex items-end justify-between gap-4 pt-4 border-t border-slate-200/60 dark:border-white/[0.05]">
              <div>
                <p className="text-sm font-semibold text-slate-900 dark:text-white leading-tight">{t.name}</p>
                <p className="text-[10px] font-mono text-slate-400 dark:text-slate-500 mt-0.5">{t.role} · {t.location}</p>
              </div>
              <div className="text-right shrink-0">
                <p className={`text-base font-bold leading-tight ${t.hue === 'cyan' ? 'text-cyan-500 dark:text-cyan-400' : 'text-blue-600 dark:text-blue-400'}`}>{t.metric}</p>
                <p className="text-[9px] font-mono text-slate-400 dark:text-slate-500 tracking-wider uppercase mt-0.5">{t.metricLabel}</p>
              </div>
            </div>
          </div>
        ))}
